...
export default class resultSection {
  ...
  result(user) {
    ...
    fetchAPI.postPaciente(user).then((paciente) => {
      ...
      this.paciente = paciente;
      this.paciente.nombresMetabolitos = [
        "3HC-O-Gluc",
        "Cotinine-N-Gluc",
        "3HC",
        "Cotinine",
  ...
  resultSkip() {
    ...
    fetchAPI.postPacienteSkip(this.paciente).then((paciente) => {
      document.getElementById(
        "result-description"
      ).innerHTML = `Dependencia ${paciente.dependencia}`;
      document.getElementById(
        "recomendacion-titulo"
      ).innerHTML = `Recomendaciones para personas con dependencia ${paciente.dependencia}:`;
      document.getElementById("recomendacion-texto").innerHTML =
